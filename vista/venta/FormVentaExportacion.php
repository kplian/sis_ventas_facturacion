...
<script>
	Phx.vista.FormVentaExportacion = {
		...
		constructor : function(config) {
			...
			this.Atributos[this.getIndAtributo('valor_bruto')].config.readOnly = false;
		...
		eventosExtras : function() {
			this.Cmp.valor_bruto.on('change', function(field) {
				this.calcularTotalesExtras()
			}, this);
			this.Cmp.transporte_fob.on('change', function(field) {
				this.calcularTotalesExtras()
			}, this);
			this.Cmp.seguros_fob.on('change', function(field) {
				this.calcularTotalesExtras()
			}, this);
			this.Cmp.otros_fob.on('change', function(field) {
				this.calcularTotalesExtras()
			}, this);
			this.Cmp.transporte_cif.on('change', function(field) {
				this.calcularTotalesExtras()
			}, this);
			this.Cmp.seguros_cif.on('change', function(field) {
				this.calcularTotalesExtras()
			}, this);
			this.Cmp.otros_cif.on('change', function(field) {
				this.calcularTotalesExtras()
			}, this);
		...
		onInitAdd : function(r, i) {
			...
			console.log('record.data.requiere_descripcion', record.data.requiere_descripcion)
		},
		...
		buildComponentesDetalle : function() {
			...
			this.detCmp = {
				...
				'id_producto' : new Ext.form.ComboBox({
					...
					forceSelection : true,
					tpl : new Ext.XTemplate('<tpl for="."><div class="x-combo-list-item">', '<tpl if="tipo == \'formula\'">', '<p><b>Medico:</b> {medico}</p>', '</tpl>', '<p><b>Nombre:</b> {nombre_producto}</p><p><b>Descripcion:</b> {descripcion}</p><p><b>Precio:</b> {precio}</p></div></tpl>'),
					typeAhead : false,
					triggerAction : 'all',
					lazyRender : true,
					mode : 'remote',
		...
		onSubmit : function(o) {
			...
			tmp.precio_total = Math.round(tmp.precio_total * 100) / 100;
			
			
			
			if (tmp.precio_total * 1 != this.Cmp.total_cif.getValue() * 1) {
				alert('el total de la venta no iguala con el total CIF');
				return;
			}
			...
			me.argumentExtraSubmit = {
				...
				'formas_pago' : JSON.stringify(formapa, function replacer(key, value) {
					if ( typeof value === 'string') {
						return String(value).replace(/&/g, "%26")
					}
					return value;
			...
			if (i > 0 && !this.editorDetail.isVisible()) {
				Phx.vista.FormVentaExportacion.superclass.onSubmit.call(this, o);
			} else {
