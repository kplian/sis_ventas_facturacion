...
CREATE OR REPLACE FUNCTION vef.f_lista_funcionario_emisor (
...
***************************************************************************
 SCRIPT: 		vef.f_lista_funcionario_emisor
...
BEGIN
    ...
    v_nombre_funcion ='vef.f_lista_funcionario_emisor';
    ...
    from vef.tventa v   
    inner join vef.tsucursal_usuario sucu on sucu.id_punto_venta = v.id_punto_venta and sucu.tipo_usuario = 'emisor'    
    inner join segu.tusuario usu on sucu.id_usuario = usu.id_usuario
    inner join orga.tfuncionario fun  on fun.id_persona = usu.id_persona
    where v.id_estado_wf = p_id_estado_wf;
    
    IF v_lista_funcionarios = '-1' THEN
       raise exception 'este punto de venta no tiene configurado un funcionario  emisor';
    END IF;
